...
def test_builtin_metadata_and_frozen_command(harness: Harness) -> None:
    ...
    # Built-ins plus the /resume alias for /sessions.
    assert commands[: len(BUILTIN_COMMANDS)] == list(BUILTIN_COMMANDS)
    assert [command.name for command in commands[len(BUILTIN_COMMANDS) :]] == ["resume"]
...
def test_resume_alias_canonicalizes_lists_metadata_and_is_reserved(harness: Harness) -> None:
    registry = CommandRegistry(harness)
    assert registry.canonical_name("resume") == "sessions"
    alias = registry.get("resume")
    assert alias is not None and alias.name == "resume" and alias.source == "harness"
    assert alias.description == "Alias for /sessions. List and resume local sessions"
    # The alias appears after the built-ins and cannot be re-registered.
    assert [command.name for command in registry.list()][len(BUILTIN_COMMANDS) :] == ["resume"]
    with pytest.raises(ValueError, match="already registered or reserved"):
        registry.register("resume", "shadow the alias", prompt="bad")
    with pytest.raises(ValueError, match="already registered or reserved"):
        registry.register_alias("resume", "sessions")
    with pytest.raises(ValueError, match="already registered or reserved"):
        registry.register_alias("help", "sessions")


@pytest.mark.parametrize("alias", ["", "/rev", "Rev", "1rev", "rev:setup", "a" * 65, "rev\n"])
def test_register_alias_validates_name(harness: Harness, alias: str) -> None:
    with pytest.raises(ValueError, match="aliases must be lowercase"):
        CommandRegistry(harness).register_alias(alias, "sessions")


def test_register_alias_validates_target_and_canonicalizes(harness: Harness) -> None:
    registry = CommandRegistry(harness)
    with pytest.raises(ValueError, match="Unknown alias target"):
        registry.register_alias("missing", "does-not-exist")
    registry.register("review", "review changes", prompt="Review changes")
    registry.register_alias("look", "review")
    assert registry.canonical_name("look") == "review"
    assert registry.get("look") == Command("look", "Alias for /review. review changes", source="harness")
    # Aliases to other aliases resolve to the canonical target.
    registry.register_alias("peek", "resume")
    assert registry.canonical_name("peek") == "sessions"
    peek = registry.get("peek")
    assert peek is not None and peek.description.startswith("Alias for /sessions.")
    assert "peek" in [command.name for command in registry.list()]


@pytest.mark.requires_posix
def test_execute_canonicalizes_alias_before_builtin_dispatch(harness: Harness) -> None:
    registry = CommandRegistry(harness)

    async def scenario() -> None:
        await harness.initialize()
        try:
            # /resume canonicalizes to the client-handled /sessions built-in and
            # must never run a model.
            with pytest.raises(ValueError, match="client-handled"):
                await registry.execute("resume", "")
            with pytest.raises(ValueError, match="client-handled"):
                await registry.execute("sessions", "")
        finally:
            await harness.close()
...
def test_list_is_live_deduplicated_and_does_not_load_or_call(harness: Harness, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    commands = registry.list()
    assert len(commands) == len(set(commands)) == len(BUILTIN_COMMANDS) + 1 + 5
...
def test_callbacks_can_use_public_harness_methods_without_model_calls(harness: Harness) -> None:
    ...
    async def scenario() -> None:
        harness.config.profiles["reviewer"] = AgentProfile(mode="reviewer")
        registry = CommandRegistry(harness)
...
def test_commands_source_has_no_ui_or_runtime_imports() -> None:
    source = Path(__file__).parents[2] / "src/nagents/harness/commands.py"
