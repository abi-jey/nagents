...
def test_read_only_design_never_starts_mcp_subprocess(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    async def drive() -> None:
        design = parse(STARTER)
        ...
        async def forbidden(self: MCPManager, server: object) -> None:
            added.append(server)
            raise AssertionError("read-only designs must not start MCP subprocesses")

        monkeypatch.setattr(MCPManager, "add_server", forbidden)
        try:
            with pytest.raises(PermissionError, match="Read-only agents cannot start MCP"):
                await harness.initialize()
            assert added == [] and not harness.mcp.is_connected
        finally:
            await harness.close()
