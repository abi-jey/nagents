...
@pytest.mark.requires_posix
def test_designed_channel_harness_preserves_read_only_floor(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    with site(tmp_path, monkeypatch) as app:
        app.configure(auto_reply=True)
        design = parse(STARTER)
        design.channels = {"fixture": "assistant"}
        ...
        session = app.bindings()["chat-a"]
        assert app.client.portal is not None
        # A later workspace preference may clear read_only, but the admin floor
        # captured at startup must still bind assembled designed harnesses.
        app.state.harness.config.read_only = False
        app.state.harness._permission_ceiling = "reviewer"
        agent, source = app.client.portal.call(app.state.designed_channels.definition, session)
        harness, _ = app.client.portal.call(app.state.designed_channels.assemble, session, agent, source, Recorder())
        try:
            assert harness._permission_ceiling == "reviewer" and harness.mode == "reviewer"
        finally:
            app.client.portal.call(harness.close)


@pytest.mark.requires_posix
def test_designer_test_dock_config_preserves_read_only_floor(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    with site(tmp_path, monkeypatch) as app:
        app.state.harness.config.read_only = False
        app.state.harness._permission_ceiling = "reviewer"
        assert Designer(app.state).config.read_only is True
