...
def test_responsive_layout(
    ...
    async def scenario() -> None:
        ...
        async with app.run_test(size=size) as pilot:
            ...
            assert not app.query("#topbar, #brand, #workspace, #model, #profile, #compose-label, #shortcuts")
            assert app.query_one("#main").region.y == int(demo)
            if demo:
            ...
            configuration = str(app.query_one("#rail-config", Static).content)
            assert backend.config.model in configuration
            assert f"Profile: {backend.config.agent}" in configuration
            assert app.query_one("#status").region.height == 1
            footer = app.query_one("#footer")
            compose_area = app.query_one("#compose-area")
            # The status box is merged into the message box, sharing its fill
            # without a separate round frame.
            ...
            assert footer.region.bottom <= size[1]
            ...
            assert app.query_one("#welcome").region.bottom < composer.region.y
            ...
            assert not app.query("#topbar, #brand, #workspace, #model, #profile, #compose-label, #shortcuts")
            assert app.query_one("#main").region.y == int(demo)
            assert composer.region.bottom <= 19
...
def test_resume_alias_opens_local_picker_without_model_call(tmp_path: Path) -> None:
    async def scenario() -> None:
        backend = FakeHarness(tmp_path)
        app = make_app(backend)
        async with app.run_test() as pilot:
            ...
            await idle(app, pilot)
            # /resume canonicalizes to the local /sessions picker and never
            # reaches the model.
            assert isinstance(app.screen, ChoiceModal)
            assert app.screen.choices[0][0] == "__new__"
            assert any(choice[0] == "saved-1" for choice in app.screen.choices)
...
def test_commands_models_profiles_and_errors(tmp_path: Path) -> None:
    async def scenario() -> None:
        backend = FakeHarness(tmp_path)
        backend.config.profiles["reviewer"] = AgentProfile(mode="reviewer")
        app = make_app(backend)
        async with app.run_test() as pilot:
            await idle(app, pilot)
...
def test_help_and_quit_remain_available_while_busy(tmp_path: Path) -> None:
    async def scenario() -> None:
        ...
        async with app.run_test() as pilot:
            ...
            assert backend.running
            await pilot.press("escape")
            # /resume canonicalizes to the local /sessions builtin: while busy it
            # is classified as a builtin (blocked), never queued or sent to model.
            await send(app, pilot, "/resume")
            await pilot.pause()
            assert not isinstance(app.screen, ChoiceModal)
            assert "Already working" in str(app.query_one("#status", Static).content)
            assert backend.prompts == ["wait"]
...
@pytest.mark.parametrize("selected", ["assistant", "audit"])
...
def test_background_config_and_visible_tool_states(tmp_path: Path, background: str) -> None:
    async def scenario() -> None:
        ...
        async with app.run_test(size=(132, 38)) as pilot:
            ...
            # Status variants pair painted message foregrounds with the message
            # surface; #rail-activity keeps the native/outside palette.
            for text, state, token in (
                ("Running", "working", "ngn-message-tool"),
                ("Ready", "ready", "ngn-message-success"),
                ("Careful", "warning", "ngn-message-warning"),
            ):
                app._status(text, state=state)
                await pilot.pause()
                assert app.query_one("#status").styles.color == Color.parse(variables[token])
            app._status("Failure", error=True)
            await pilot.pause()
            assert app.query_one("#status").styles.color == Color.parse(variables["ngn-message-error"])
