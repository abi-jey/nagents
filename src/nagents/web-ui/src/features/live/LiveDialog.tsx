...
import type { LiveCapability, LivePhase } from "./types.js";
import "./live.css";
...
export function LiveDialog({ token, close }: { token: string; close: () => void }) {
  ...
  const provider = config?.provider === "azure_openai_compatible_v1" ? "Azure OpenAI" : config?.provider === "openai_compatible" ? "OpenAI-compatible" : config?.provider === "openai" ? "OpenAI" : config?.provider || "Hosted intelligence";
  ...
  async function refresh() {
    ...
    try {
      ...
      if (!abort.signal.aborted) { setConfig(next); setVoice((current) => current || next.voice); }
    } catch (cause) {
  ...
  return <dialog ref={dialog} className="live-dialog" aria-labelledby="live-title" onCancel={(event) => { event.preventDefault(); close(); }}>
    <header className="live-header">
      ...
      <button type="button" className="live-close" aria-label={active ? "End voice and close" : "Close voice studio"} title={active ? "End voice and close" : "Close"} onClick={close}><Icon name="close" size={20} /></button>
    </header>
    <div className="live-layout">
      <section className={`live-stage ${connected ? "is-connected" : ""}`} aria-label="Voice connection">
        ...
        <div className="live-actions">
          ...
          </> : <button type="button" className="live-connect" disabled={loading || !!loadError || !config?.available || !!unavailable} onClick={() => { follow.current = true; void controller.start(voice); }}><Icon name="wave" size={21} />{loading ? "Checking connection…" : state.phase === "idle" ? "Start conversation" : "Start a new conversation"}<span aria-hidden="true">↗</span></button>}
        </div>
        ...
        <p className="live-audio-note">{connected ? `${state.micMuted ? "Microphone muted" : "Microphone on"} · ${state.outputMuted ? "Speaker muted" : "Speaker on"}` : "Your microphone turns on only when you connect."}</p>
        {(unavailable || loadError || state.error || state.notice) && <div className={`live-feedback ${state.error || loadError ? "has-error" : ""}`} role={state.error || loadError ? "alert" : "status"}>
          ...
          {!active && <button type="button" disabled={loading} onClick={() => void refresh()}>Refresh connection settings</button>}
        </div>}
        <div className="live-configuration"><label htmlFor="live-voice">VOICE<select id="live-voice" value={voice} disabled={active || !config || loading} onChange={(event) => setVoice(event.target.value)}>{[...new Set([config?.voice || "marin", ...(config?.voices || [])])].map((name) => <option key={name} value={name}>{name.charAt(0).toUpperCase() + name.slice(1)}</option>)}</select></label><div><span>VOICE MODEL</span><strong>{config?.model || "gpt-live-1"}</strong></div><div><span>BACKEND</span><strong title={config?.backend_model}>{config?.backend_model || "Configured by server"}</strong></div></div>
      </section>
      <section className="live-transcript-panel" aria-label="Live captions">
