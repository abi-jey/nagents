...
`dictation:`, or `live:` mapping. Named agent profiles are the only nested mapping:
...
### GPT-Live in ngn serve

These startup settings configure a dedicated hosted voice agent, independent of
the chat provider, selected profile, and dictation. They use the same trusted YAML
layers and `NGN_*` precedence as other scalar fields. Restart `ngn serve` after
changing them; browser Settings do not persist Live configuration.

| Field | YAML type | Default | Accepted values and meaning |
| --- | --- | --- | --- |
| `live_enabled` | boolean | `false` | Enables browser GPT-Live. Starting a call still requires an explicit browser action. Offline demo mode always disables it. |
| `live_provider` | string | `"openai"` | Dedicated voice provider. `openai`, `openai_compatible`, and `azure_openai_compatible_v1` support Live; other known provider names produce an unavailable capability reason. |
| `live_model` | string | `"gpt-live-1"` | Voice model/deployment ID, 1–128 characters, matching `[A-Za-z0-9][A-Za-z0-9_.:/-]*`. This is GPT-Live, not the separate Realtime API. |
| `live_backend_model` | string | `"gpt-5.6-luna"` | Hosted Responses backend model/deployment. Defaults to the library's `LiveConfig.backend_model`; same ID validation as `live_model`. |
| `live_voice` | string | `"marin"` | `"marin"` or `"cedar"`. The browser can choose either for a new call. |
| `live_base_url` | string | `""` | Empty uses OpenAI's `https://api.openai.com/v1`. Azure v1 requires an explicit API prefix. Custom prefixes require HTTPS, except loopback HTTP for local development; credentials, query parameters, fragments, and generation-route suffixes are rejected. |
| `live_api_key_env` | string | `"OPENAI_API_KEY"` | Server environment-variable name matching `[A-Za-z_][A-Za-z0-9_]*`; never the key value. Only this variable supplies the Live credential. |

The selected endpoint must support GPT-Live WebRTC provisioning, WebSocket
sideband controls, and the chosen hosted Responses backend. An OpenAI-compatible
chat API alone does not provide those capabilities. Azure v1 uses the existing
`FoundryProvider` API-key transport. Capability discovery checks local setup,
not account access or endpoint reachability; creation errors appear when connecting.

The conventional `OPENAI_API_KEY` works even when chat uses Anthropic, Gemini, or
ChatGPT/Codex login. Live does not borrow saved OAuth tokens or provider-login
credentials. Use a dedicated `live_api_key_env` when voice should have a separate
key. See [GPT-Live setup and lifecycle](ngn-web.md#gpt-live-voice-conversations).
...
`NGN_DICTATION_*` fields, all seven `NGN_LIVE_*` fields, as well as provider,
model, limits, and state storage.
...
different files remain available, but a later `profiles.audit` entry constructs a
